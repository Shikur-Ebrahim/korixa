    ...
    <section className="px-4 py-10 sm:px-6 sm:py-14">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {reasons.map((reason) => {
            const card = (
              <Card key={reason.title} hover className={`text-center${reason.href ? " cursor-pointer transition hover:border-primary/40" : ""}`}>
            ...
            return reason.href ? (
              <Link key={reason.title} href={reason.href}>
                {card}
              </Link>
            ) : (
              <div key={reason.title}>{card}</div>
            );
          })}
        </div>
      </div>
