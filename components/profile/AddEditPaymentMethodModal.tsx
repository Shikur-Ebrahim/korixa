    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
        ...
        >
          ...
          <div className="p-4 overflow-y-auto flex-1">
            <form id="payment-form" onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-[#848e9c]">Payment Type</label>
                <select
                  value={type}
                  onChange={(e) => setType(e.target.value as PaymentMethodType)}
                  className="w-full rounded-xl border border-white/[0.06] bg-[#0b0e11] px-4 py-3 text-[14px] font-medium text-white focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-all"
                  required
                >
              ...
              {type === "other" && (
                <div className="space-y-1.5">
                  <label className="text-[13px] font-medium text-[#848e9c]">Bank Name</label>
                  <input
                    type="text"
                    value={bankName}
                    onChange={(e) => setBankName(e.target.value)}
                    placeholder="Enter full bank name"
                    className="w-full rounded-xl border border-white/[0.06] bg-[#0b0e11] px-4 py-3 text-[14px] font-medium text-white focus:border-primary focus:outline-none transition-all placeholder:text-white/20"
                    required
                  />
                </div>
              )}
              ...
              {/* Account Holder Name */}
              <div className="space-y-1.5">
                <label className="text-[13px] font-medium text-[#848e9c]">Account Holder Name</label>
                <input
                  type="text"
              ...
              {isTelebirr ? (
                <div className="space-y-1.5">
                  <label className="text-[13px] font-medium text-[#848e9c]">Phone Number</label>
                  <input
                    type="tel"
                    value={phoneNumber}
                ...
                <div className="space-y-1.5">
                  <label className="text-[13px] font-medium text-[#848e9c]">Account Number</label>
                  <input
                    type="text"
                    value={accountNumber}
          ...
          <div className="p-4 border-t border-white/[0.06] bg-[#161a1e]">
            <button
              form="payment-form"
              type="submit"
              disabled={loading}
              className="w-full flex min-h-[52px] items-center justify-center rounded-xl bg-primary text-[15px] font-bold text-[#0b0e11] disabled:opacity-50 transition-all active:scale-[0.98]"
            >
